...
import (
	...
	"github.com/ducconit/gomail/inbucket"
...
func getConfig() *SMTPConfig {
	server := os.Getenv("MAIL_HOST_TEST")
	if server == "" {
		server = "localhost"
	}
	port := os.Getenv("MAIL_PORT_TEST")
	if port == "" {
		port = "2500"
	}
...
func TestSendMailUsingConfig(t *testing.T) {
	...
	var resultsMailbox inbucket.JSONMessageHeaderInbucket
	err3 := inbucket.RetryInbucket(5, func() error {
		var err error
		resultsMailbox, err = inbucket.GetMailBox(emailTo)
		return err
	})
	if err3 != nil {
		t.Log(err3)
		...
		if len(resultsMailbox) > 0 {
			...
			resultsEmail, err := inbucket.GetMessageFromMailbox(emailTo, resultsMailbox[0].ID)
...
func TestSendMailUsingConfigAdvanced(t *testing.T) {
	...
	//Delete all the messages before check the sample email
	inbucket.DeleteMailBox("test2@example.com")
	...
	//Check if the email was send to the right email address
	var resultsMailbox inbucket.JSONMessageHeaderInbucket
	err = inbucket.RetryInbucket(5, func() error {
		var mailErr error
		resultsMailbox, mailErr = inbucket.GetMailBox(mail.smtpTo)
		return mailErr
	})
	...
	resultsEmail, err := inbucket.GetMessageFromMailbox(mail.smtpTo, resultsMailbox[0].ID)
	require.NoError(t, err)
